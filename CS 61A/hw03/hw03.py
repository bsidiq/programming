...
def helper(n, index, current, direction):
    if index == n:
        return current
    else:
        if index % 8 == 0 or num_eights(index):
            return helper(n, index + 1, current - direction, -direction)
        else:
            return helper(n, index + 1, current + direction, direction)
# use without a helper function if you have time
...
def count_coins(change):
    ...
    """
    curr = 1
    return count_coins_helper(change, curr)

def count_coins_helper(change, curr):
    if change == 0:
       return 1
    if change < 0 or curr == None:
       return 0
    return count_coins_helper(change - curr, curr) + count_coins_helper(change, ascending_coin(curr))
