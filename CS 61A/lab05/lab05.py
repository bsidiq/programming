def factors_list(n):
    ...
    """
    #all_factors = []
    all_factors = [x for x in range(1, n) if n % x ==0 and x < n]
    return all_factors
...
def make_city(city_name, city_lat, city_lon):
...
def closer_city(lat, lon, city_a, city_b):
    ...
    'Bucharest'
    """
    new_city_distance = make_city("new", lat, lon)
    if (distance(city_a, new_city_distance) 
        < distance(city_b, new_city_distance)):
