class VendingMachine:
    ...
    def vend(self):
        if not self.stock:
            ...
            return 'You must add ${0} more funds.'.format(amount_left)
        
        elif self.balance > self.price:
            remaining_funds = self.balance - self.price
            self.balance = 0
            self.stock -= 1
            return 'Here is your {0} and ${1} change.'.format(self.name, remaining_funds)
        else: 
            self.stock-= 1
...
def two_list(vals, amounts):
    ...
    """
    vals.reverse(), amounts.reverse()
    index, link = 0, Link.empty
    for b in amounts:
        for i in range(b):
            link=Link(vals[index], link)
        index +=1
    return link
