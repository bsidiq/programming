...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8
            if place.ant.can_contain(self) and place.ant.ant_contained is None:
...
class HungryAnt(Ant):
    ...
    def eat_bee(self, bee):
        killed_bee = random_bee(bee)
        if killed_bee != None:
            killed_bee.reduce_health(killed_bee.health)
...
# END Problem 12
    ...
    def action(self, gamestate):
        if not self.is_truequeen:
            ...
            while back:
                if back.ant:
                    if not back.ant.buffed:
                        back.ant.damage *= 2
                        back.ant.buffed = True
                    elif back.ant.is_container() and back.ant.contained_ant:
                        if not back.ant.ant_contained.buffed:
                            back.ant.ant_contained.damage *= 2
                            back.ant.ant_contained.buffed = True
                back = back.exit
