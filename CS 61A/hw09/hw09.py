...
def cs_classes(post):
    ...
    True
    """
    return bool(re.search('[Cc][Ss][0-9]|([\s][0-9])+[ABCabc]*', post))
...
def area_codes(text):
    ...
    []
    """
    return re.findall(r'(\[0-9]{3}|[0-9]{3})[ -\.]?[0-9]{3}',text)
...
def most_common_code(text):
    ...
    """
    count=[]
    most_common = re.findall(r'(\([0-9]{3}\)|[0-9]{3})[ -\.]?[0-9]{3}[ -\.]?[0-9]{4}',text)
    for i in range(len(most_common)):
        if most_common[i][0]=="(":
            most_common[i]= most_common[i][1:-1]
    
    for i in most_common:
        count.append(most_common.count(i))
