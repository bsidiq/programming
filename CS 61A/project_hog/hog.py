...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def bigannouncement(score0, score1):
        #nonlocal last_score, running_high #p1, p2 respectively
        #current_score = 0
        if who == 0:
            score = score0
        else:
        ...
        if score-last_score> running_high: 
            new_running_high = score-last_score
            print(f"{new_running_high} point(s)! That's a record gain for Player {who}!")
        else: 
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    count, max = 1, 0
    while count <= 10:
        roll_dice_av = make_averaged(roll_dice, trials_count)
        temp = roll_dice_av(count, dice)
        if temp > max:
            max = temp
            max_score = count
        count += 1
...
def hog_pile_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    if picky_piggy(opponent_score)+score == opponent_score:
        ...
        num_rolls = picky_piggy_strategy(score, opponent_score, cutoff, num_rolls)
    return num_rolls
      
   

    #elif hog_pile(score, opponent_score) == score * 2:
        #num_rolls = 0

    
    
    #elif hog_pile(score, opponent_score) >= cutoff:
        #num_rolls = 0
